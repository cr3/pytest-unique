...
import pytest_unique.registry
from pytest_unique.registry import (
    registry_add,
    registry_get,
    registry_load,
    registry_remove,
...
def test_registry_load():
    ...
    with patch.object(
        ...
        mock_entry_points.return_value = iter([
            EntryPoint(
                "registry", "pytest_unique.registry", "pytest_unique.group"
            ),
        ])
        registry = registry_load("group")
        assert "registry" in registry["group"]
        assert registry["group"]["registry"] == pytest_unique.registry
...
def test_registry_load_ignore():
    ...
    with patch.object(
        ...
        mock_entry_points.return_value = iter([
            EntryPoint(
                "error", "pytest_unique.registry", "pytest_unique.group"
            ),
        ])
        registry = registry_load("group")
...
def test_registry_add():
    ...
    assert registry["group"]["name"] is True
...
def test_registry_get():
    ...
    registry = registry_add("group", "name", True)
    assert registry_get("group", "name", registry) is True
